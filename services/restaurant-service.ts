...
class RestaurantService {
  ...
  async get() {
    const requestString =
      'select u.id, u.username, (select count(*) from menu_detail where menu_id = m.id) "details_count", ' +
      '(select count(*) from review r3 where reviewee_id = u.id) "review_count" ' +
      'from "user" u ' +
    ...
    const request = await pool.query(requestString);
